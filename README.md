...
```sh
opkg update && opkg install curl ca-bundle jq && curl -fsSL -o /opt/tmp/awg-manager-tunnel-access.sh https://raw.githubusercontent.com/genaRijoff/awgm_tun_wgX/main/awg-manager-tunnel-access.sh && chmod +x /opt/tmp/awg-manager-tunnel-access.sh && /opt/tmp/awg-manager-tunnel-access.sh
```
## Запуск 
```sh
